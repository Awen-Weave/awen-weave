...
- **Next for S1**: Huw commits + pushes to `Awen-Weave/awen-weave`; deploy to craidd (`git pull` + `pip install -e .`); run `craidd-snapshot dolgellau-gazetteer` for the real snapshot; append signal; **hand S2 to the CHI chat** (`pull_tref.py` consumes the snapshot). Later: `duckdb` read of the Town Dataset's own run-manifest for `ran_at_utc` if/when it emits one (currently git HEAD).
